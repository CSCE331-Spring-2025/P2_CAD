...
/**
 * A panel for managing menu items in a restaurant management system.
 * <p>
 * This panel displays menu items in a table format and provides 
 * functionality to add, update, and delete menu items from the database.
 * The panel features an orange-themed UI with a white table for better readability.
 * 
...
public class MenuPanel extends JPanel {
    ...
    /**
     * Constructs a new MenuPanel with a table showing menu items and CRUD operation buttons.
     * <p>
     * The panel uses a BorderLayout with the menu table in the center and action buttons
     * at the bottom. The UI is styled with an orange color scheme (#FFC364) to match
    ...
    /**
     * Loads menu item data from the database and populates the table.
     * <p>
     * This method clears any existing data in the table and fetches fresh data
     * from the PostgreSQL database. It displays the menu ID, name, price, and 
     * seasonal status for each menu item, ordered by menu ID.
     * <p>
     * Any null seasonal values in the database are treated as false.
     * 
     * @see Connection
     * @see ResultSet
     */
     ...
     * <p>
     * This method presents a form with fields for the item name, price, and
     * a checkbox for seasonal status. The user input is validated to ensure
     * the name is not empty and the price is a valid positive number.
     * <p>
     * A new menu ID is automatically generated by finding the next available ID.
     * After successful addition, the table is refreshed and a confirmation message 
     * is displayed.
     * 
     * @see JOptionPane
    ...
    /**
     * Determines the next available menu ID for a new menu item.
     * <p>
     * This method queries the database for the maximum existing menu ID 
     * and increments it by one to get the next available ID. If no menu
     * items exist in the database, it returns 1 as the starting ID.
     * 
     * @return the next available menu ID, or -1 if an error occurs
     * @see Statement
     * @see ResultSet
     */
    ...
    /**
     * Displays a dialog for updating the selected menu item.
     * <p>
     * This method checks if an item is selected and presents a form pre-populated
     * with the current values of the selected item. The user can modify the name,
     * price, and seasonal status. Input validation ensures the name is not empty
     * and the price is a valid positive number.
     * <p>
     * After successful update, the table is refreshed and a confirmation message
     * is displayed.
     * 
     * @see JOptionPane
     * @see PreparedStatement
     ...
     * <p>
     * This method checks if an item is selected in the table and asks the user
     * to confirm the deletion. If confirmed, the item is deleted from the database
     * based on its menu ID.
     * <p>
     * After successful deletion, the table is refreshed and a confirmation message
     * is displayed.
     * 
     * @see JOptionPane
     * @see PreparedStatement
    ...
    /**
     * Main method for testing the MenuPanel independently.
     * <p>
     * Creates a JFrame with the MenuPanel added to it and displays it.
     * The frame is configured with an orange background to match the panel's theme
